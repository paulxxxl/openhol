<tr>
  ...
  <td class="simplecheckout-table-form-right">
    ...
    <?php } else { ?>
      <input type="<?php echo $type == 'date' ? 'text' : $type ?>" name="<?php echo $name ?>" id="<?php echo $id ?>" value="<?php echo $value ?>" placeholder="<?php echo $placeholder ?>" <?php echo $attrs ?> <?php echo $reload ? 'data-onchange="reloadAll"' : 'data-reload-payment-form="true"'?>>
    <?php } ?>
    <?php if (!empty($rules)) { ?>
