...
class ModelToolSimpleGeo extends Model {
    ...
    public function getGeoList($city) {
        ...
        if (!$geo_data) {
            ...
            /*$sql = "SELECT
                f1.fias_id AS id,
                CONCAT_WS(', ', CONCAT(f1.shortname, ' ', f1.offname), CONCAT(f2.offname, ' ', f2.shortname), CONCAT(f3.offname, ' ', f3.shortname)) AS full_name,
            ...
            WHERE f1.offname LIKE '" . $this->db->escape($city) . "%'
          AND (f1.level = 6 OR f1.level = 4 OR (f1.level = 1 AND f1.shortname = 'г.'))
          ORDER BY f1.level, f2.level, f3.level, f1.shortname LIMIT 100";*/
